...
export default function FoliarRound({ isEmbedded = false }: { isEmbedded?: boolean }) {
  const now = new Date();
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Fetch custom settings if available
      const currentEstId = !isUserAdmin ? profile?.estate_id : (estateFilter !== 'all' ? estateFilter : null);
      if (currentEstId) {
        const { data: setRes } = await supabase.from('foliar_settings').select('settings').eq('estate_id', currentEstId).maybeSingle();
        if (setRes?.settings?.cycle_interval_days) {
          setIntervalDays(setRes.settings.cycle_interval_days);
        }
      }

      const [logsRes, musterRes] = await Promise.all([
        supabase.from('foliar_logs').select('*'),
      ...
      const dbMusterFoliar = (musterRes.data || []).filter((m: any) =>
        m.task && String(m.task).toLowerCase().includes('foliar')
      );
      ...
      const foliarLogDatesByBlock = new Set<string>();
      ...
      (logsData || []).forEach((log: any) => {
        ...
        foliarLogDatesByBlock.add(`${bId}_${dateKey}`);

        logsByBlock[bId].push({
          ...log,
          date: dateKey,
          area: Number(log.area_covered || log.area || 0),
          qty: Number(log.liters_sprayed || log.qty || 0),
          labours: Number(log.labours || (log.worker_id ? 1 : 0)) || 1,
          type: log.chemical_used || log.type || 'Standard Foliar'
        });
      });

      (dbMusterFoliar || []).forEach((m: any) => {
        ...
        if (foliarLogDatesByBlock.has(`${bId}_${dateKey}`)) return;
        ...
        logsByBlock[bId].push({
          ...
          qty: Number(m.units || 0) * 1.5,
          labours: Number(m.workers || m.labours || (m.worker_id ? 1 : 0)) || 1,
          type: m.chemical || 'Standard Foliar'
        });
      });
      ...
      mappedFields.forEach((f: any) => {
        ...
        const tot: any = {
          id: f.id, area: 0, qty: 0, labours: 0,
  ...
  const exportToExcel = () => {
    const data = filteredFields.map(f => {
      ...
      return {
        ...
        'Total Sprayed Area (Ac)': (tot.area || 0).toFixed(2),
        'Total Spray Qty (Ltrs)': (tot.qty || 0).toFixed(1),
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Foliar Status");
    XLSX.writeFile(wb, `foliar_round_report_${startDate}_to_${endDate}.xlsx`);
  };
  ...
  const exportToPDF = () => {
    ...
    doc.text("Foliar Round Operational Report", 14, 20);
    ...
    autoTable(doc, {
      startY: 40,
      head: [['Block', 'Total Area', 'Sprayed Area', 'Volume', 'Next Due', 'Status']],
      body: tableData,
      theme: 'grid',
    ...
    doc.save(`foliar_round_report_${startDate}_to_${endDate}.pdf`);
  };
    ...
    <div className="pb-16 space-y-4">
      ...
      {!isEmbedded && (
        <div className="flex flex-col gap-1">
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">Foliar Round Monitor</h1>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          {
            label: "Spray Volume",
            val: filteredTotals.qty.toFixed(0),
            unit: "Ltrs",
            icon: Activity,
          ...
          },
          {
            ...
            unit: "blocks",
            icon: AlertCircle,
            color: statusCounts.overdue > 0 ? "text-red-600 dark:text-red-400" : "text-sky-600 dark:text-sky-400",
            bg: statusCounts.overdue > 0 ? "bg-red-50 dark:bg-red-900/20" : "bg-sky-50 dark:bg-sky-900/20"
          },
        ].map((s, i) => (
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          ...
          {isUserAdmin && (
            <select
              ...
              className="h-9 px-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg text-sm font-semibold text-slate-700 dark:text-slate-300 outline-none focus:border-sky-500 min-w-[140px]"
            >
          ...
          <div className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-50 dark:bg-slate-950 rounded-lg border border-slate-200 dark:border-slate-800 text-xs font-semibold text-slate-700 dark:text-slate-300">
            ...
            <select
              ...
              className="bg-transparent font-bold text-sky-700 dark:text-sky-400 outline-none cursor-pointer"
            >
          ...
          <div className="relative w-full sm:w-64">
            ...
            <input
              ...
              className="w-full h-9 pl-9 pr-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg text-sm font-semibold text-slate-700 dark:text-slate-300 outline-none focus:border-sky-500 placeholder:text-slate-400"
            />
          </div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <div className="relative">
              ...
              <select
                ...
                className="h-9 pl-9 pr-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg text-sm font-semibold text-slate-700 dark:text-slate-300 outline-none focus:border-sky-500 cursor-pointer"
              >
      ...
      <div className="flex items-center gap-1.5 overflow-x-auto pb-1 bg-slate-50 dark:bg-slate-950 p-1.5 rounded-xl border border-slate-200 dark:border-slate-800">
        {[
          ...
          { key: 'warn', label: 'Due Soon (≤ 7 Days)', count: statusCounts.warn, badgeBg: 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 border border-amber-200 dark:border-amber-800' },
          { key: 'ok', label: 'In-Cycle / Optimal', count: statusCounts.ok, badgeBg: 'bg-sky-100 dark:bg-sky-900/30 text-sky-700 dark:text-sky-400 border border-sky-200 dark:border-sky-800' },
        ].map(tab => (
          <button
            key={tab.key}
      ...
      {loading ? (
        <div className="flex items-center justify-center py-20 gap-2 text-slate-400">
          ...
          <span className="text-sm font-medium">Loading Foliar Intelligence...</span>
        </div>
      ) : (
        <>
          ...
          {activeField ? (
                ...
                <div>
                  <div className="flex items-center gap-3">
                    ...
                    <button
                      ...
                      className="text-xs font-semibold text-sky-600 dark:text-sky-400 hover:underline flex items-center gap-1 bg-sky-50 dark:bg-sky-900/20 px-2 py-0.5 rounded-lg border border-sky-100 dark:border-sky-800"
                    >
                      Block Details <ArrowRight size={11} />
                    </button>
                  </div>
                  <p className="text-xs text-sky-600 dark:text-sky-400 font-medium mt-0.5">Foliar Application Log Matrix ({formatDate(startDate)} – {formatDate(endDate)})</p>
                </div>
                          ...
                          return (
                            <tr
                              ...
                              className={`transition-colors ${
                                row ? 'bg-sky-50/50 dark:bg-sky-950/20 hover:bg-sky-100/50 cursor-pointer' : 'hover:bg-slate-50 dark:hover:bg-slate-800/50'
            ...
            <div className="p-8 text-center bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl text-slate-400 text-xs font-medium">
              Please select a block to view foliar log matrix.
            </div>
          )}
        </>
      )}
            ...
            <div>
              ...
              <div className="py-6 space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div className="p-3.5 rounded-xl bg-sky-50 dark:bg-sky-900/20 border border-sky-100 dark:border-sky-800">
                    <p className="text-xs text-sky-600 dark:text-sky-400 font-medium">Total Sprayed</p>
                ...
                <div className="space-y-3 pt-4 border-t border-slate-200 dark:border-slate-800">
                  <h4 className="text-xs font-semibold text-slate-500 flex items-center gap-2">
                    <ShieldCheck size={14} className="text-sky-600 dark:text-sky-400" /> Block Operational Readiness
                  </h4>
                  ...
                  <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-800 space-y-2.5 text-xs">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-sky-700 dark:text-sky-400 font-bold">{(totals[selectedBlockForDetail.id]?.qty || 0).toFixed(1)} Ltrs</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-slate-600 dark:text-slate-400 font-medium">Target Interval</span>
                      <span className="text-sky-700 dark:text-sky-400 font-bold">{intervalDays} Days Cycle</span>
                    </div>
                    <div className="flex justify-between items-center">
        ...
        <div className="fixed inset-0 z-[110] bg-slate-900/50 backdrop-blur-sm flex justify-end animate-in fade-in duration-200">
          <div className="w-full max-w-md bg-white dark:bg-slate-900 h-full overflow-y-auto shadow-2xl p-6 flex flex-col justify-between border-l border-slate-200 dark:border-slate-800 space-y-6">
            <div>
              ...
              <div className="flex justify-between items-center pb-4 border-b border-slate-200 dark:border-slate-800">
                <div>
                  <span className="text-[10px] font-bold uppercase tracking-wider text-sky-600 dark:text-sky-400 bg-sky-50 dark:bg-sky-900/30 px-2 py-0.5 rounded-full border border-sky-200 dark:border-sky-800">
                    {selectedDayDetail.row.round || 'Foliar Log'}
                  </span>
              ...
              <div className="grid grid-cols-2 gap-3 mt-6">
                <div className="p-3.5 rounded-xl bg-sky-50 dark:bg-sky-900/20 border border-sky-100 dark:border-sky-800">
                  <div className="flex items-center gap-1.5 text-sky-700 dark:text-sky-400 mb-0.5">
                    <Droplets size={14} />
                    <span className="text-xs font-medium">Sprayed Area</span>
                  </div>
              ...
              <div className="space-y-2.5 p-3.5 rounded-xl bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-800 text-xs font-semibold mt-4">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-slate-500 font-normal">Spray Volume</span>
                  <span className="text-sky-600 dark:text-sky-400 font-bold">{selectedDayDetail.row.qty || 0} Ltrs</span>
                </div>
          ...
          <div className="w-full max-w-md max-h-[90vh] overflow-y-auto bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 p-6 space-y-5">
            ...
            <div className="flex justify-between items-center border-b border-slate-100 dark:border-slate-800 pb-3">
              <div className="flex items-center gap-2">
                <div className="p-2 rounded-xl bg-sky-50 dark:bg-sky-900/20 text-sky-600 dark:text-sky-400">
                  <Settings size={18} />
                </div>
                <div>
                  <h3 className="text-base font-bold text-slate-900 dark:text-white">
                    Foliar Round Settings
                  </h3>
                  <p className="text-xs text-slate-400">Configure foliar cycle & display options</p>
                </div>
              </div>
              <Button
                variant="ghost"
                  ...
                  <button
                    ...
                    className={`h-9 rounded-xl text-xs font-semibold border transition-all ${
                      intervalDays === item.val
                        ? 'bg-sky-50 dark:bg-sky-900/30 text-sky-700 dark:text-sky-400 border-sky-500 font-bold shadow-sm'
            ...
            <div className="space-y-2 pt-2 border-t border-slate-100 dark:border-slate-800">
              <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide flex items-center gap-1.5">
                ...
                Annual Foliar Round Guidelines
              </label>
              <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 space-y-2.5 text-xs text-slate-600 dark:text-slate-300">
                <div className="flex items-start gap-2">
                  ...
                  <p>
                    <strong className="text-slate-900 dark:text-white">Annual Reset:</strong> The first foliar application date in any given calendar year resets to <span className="font-semibold text-sky-600 dark:text-sky-400">Round 01</span>.
                  </p>
                </div>
                <div className="flex items-start gap-2">
                  <div className="w-1.5 h-1.5 rounded-full bg-sky-500 mt-1.5 shrink-0" />
                  <p>
                    <strong className="text-slate-900 dark:text-white">Round Progress:</strong> Subsequent foliar dates in that year increment (<span className="font-semibold text-slate-700 dark:text-slate-200">Round 02, Round 03</span>, etc.) after every interval period.
                  </p>
                </div>
                <div className="flex items-start gap-2">
                  <div className="w-1.5 h-1.5 rounded-full bg-sky-500 mt-1.5 shrink-0" />
                  <p>
                    <strong className="text-slate-900 dark:text-white">Year-End Round:</strong> The final foliar spray date of the year represents the relevant last round of that year.
                  </p>
                </div>
                <div className="flex items-start gap-2">
                  <div className="w-1.5 h-1.5 rounded-full bg-sky-500 mt-1.5 shrink-0" />
                  <p>
                    <strong className="text-slate-900 dark:text-white">Next Due Prediction:</strong> If the next projected spray date falls in the same calendar year, Next Round shows <span className="font-semibold text-amber-600 dark:text-amber-400">Round N+1</span>. If it falls into the next calendar year, Next Round automatically predicts <span className="font-semibold text-sky-600 dark:text-sky-400">Round 01</span>.
                  </p>
                </div>
              </div>
            </div>
