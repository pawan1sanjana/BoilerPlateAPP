...
export default function PluckingRound() {
  const now = new Date();
  ...
  const [viewMode, setViewMode] = useState<'combined' | 'grid' | 'matrix'>('combined');
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [logsRes, musterRes] = await Promise.all([
        supabase.from('plucking_logs').select('*'),
      ...
      if (logsRes.data && logsRes.data.length > 0) {
        ...
        const resYearLogs = await apiClient.get(`/crop/plucking-logs/year?year=${reqYear}`);
        if (resYearLogs.success && resYearLogs.data) {
          logsData = resYearLogs.data;
        }
      }

      // Strictly filter daily_muster for Plucking tasks (case-insensitive)
      const dbMusterPlucking = (musterRes.data || []).filter((m: any) =>
        m.task && String(m.task).toLowerCase().includes('pluck')
      );
      const hasRealDbLogs = (logsData && logsData.length > 0) || (dbMusterPlucking && dbMusterPlucking.length > 0);
      ...
      const pluckingLogDatesByBlock = new Set<string>();

      // Process plucking_logs
      (logsData || []).forEach((log: any) => {
        ...
        pluckingLogDatesByBlock.add(`${bId}_${dateKey}`);

        let qty = Number(log.total_kg || log.kg || log.qty || 0);
        if (qty === 0 && log.interval_weights) {
          const weights = typeof log.interval_weights === 'string' ? JSON.parse(log.interval_weights) : log.interval_weights;
          qty = Object.values(weights || {}).reduce<number>((sum: number, w: any) => sum + (parseFloat(w as string) || 0), 0);
        }
      ...
      // Process daily_muster task-wise entries for Plucking
      (dbMusterPlucking || []).forEach((m: any) => {
        ...
        if (pluckingLogDatesByBlock.has(`${bId}_${dateKey}`)) return;

        if (!logsByBlock[bId]) logsByBlock[bId] = [];
        let qty = Number(m.total_kg || m.kg || m.qty || 0);
      ...
      // Fallback sample data ONLY if no entries exist in database at all
      if (!hasRealDbLogs && mappedFields.length > 0) {
        ...
        mappedFields.forEach((f: any) => {
          ...
          while (curr <= end) {
            ...
            const acres = parseFloat(f.acres) || 5;
            const labours = Math.floor(acres * (2 + Math.random() * 2)) || 8;
            const qty = Math.round(labours * (18 + Math.random() * 10));

            logsByBlock[f.id].push({
              block_id: f.id,
              date: dateStr,
              area: Number((acres * (0.8 + Math.random() * 0.2)).toFixed(2)),
              labours: labours,
              qty: qty
            });

            curr.setDate(curr.getDate() + intervalDays + Math.floor(Math.random() * 2));
          }
        });
      }
      ...
      mappedFields.forEach((f: any) => {
        ...
        if (logs.length > 0) {
          ...
          const latestLogMonth = latestLogDate.substring(0, 7);
          const nextDueMonth = tot.nextDue.substring(0, 7);

          if (nextDueMonth !== latestLogMonth) {
  ...
  const exportToExcel = () => {
    const data = filteredFields.map(f => {
      ...
      return {
        ...
        'Latest Round Coverage (Ac)': (tot.latestRoundArea || 0).toFixed(2),
        'Total Yield (kg)': (tot.qty || 0).toFixed(2),
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Plucking Status");
    XLSX.writeFile(wb, `plucking_round_report_${startDate}_to_${endDate}.xlsx`);
  };
  ...
  const exportToPDF = () => {
    ...
    doc.text("Plucking Round Operational Report", 14, 20);
    ...
    autoTable(doc, {
      startY: 40,
      head: [['Block', 'Total Area', 'Round Coverage', 'Total Yield', 'Next Due', 'Status']],
      body: tableData,
      theme: 'grid',
      headStyles: { fillColor: [45, 138, 86], textColor: [255, 255, 255], fontStyle: 'bold' },
      alternateRowStyles: { fillColor: [245, 250, 247] },
      styles: { fontSize: 9, cellPadding: 3 }
    });

    doc.save(`plucking_round_report_${startDate}_to_${endDate}.pdf`);
  };
  ...
  return (
    <div className="pb-16 space-y-4">
      ...
      <div className="flex flex-col gap-1">
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">Plucking Round Monitor</h1>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          {
            label: "Total Harvest",
            val: filteredTotals.qty.toFixed(1),
            unit: "kg",
            icon: Sprout,
          ...
          },
          {
            ...
            val: (filteredTotals.qty / Math.max(1, filteredTotals.labours)).toFixed(1),
            unit: "kg/pax",
            icon: Activity,
          ...
          },
          {
            ...
            unit: "blocks",
            icon: AlertCircle,
            color: statusCounts.overdue > 0 ? "text-red-600 dark:text-red-400" : "text-tea-600 dark:text-tea-400",
            bg: statusCounts.overdue > 0 ? "bg-red-50 dark:bg-red-900/20" : "bg-tea-50 dark:bg-tea-900/20"
          },
        ].map((s, i) => (
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          ...
          <div className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-50 dark:bg-slate-950 rounded-lg border border-slate-200 dark:border-slate-800 text-xs font-semibold text-slate-700 dark:text-slate-300">
            ...
            <select
              ...
              className="bg-transparent font-bold text-tea-700 dark:text-tea-400 outline-none cursor-pointer"
            >
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <div className="relative">
              ...
              <select
                ...
                className="h-9 pl-9 pr-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg text-sm font-semibold text-slate-700 dark:text-slate-300 outline-none focus:border-tea-500 cursor-pointer"
              >
      ...
      <div className="flex items-center gap-1.5 overflow-x-auto pb-1 bg-slate-50 dark:bg-slate-950 p-1.5 rounded-xl border border-slate-200 dark:border-slate-800">
        {[
          ...
          { key: 'warn', label: 'Due Soon (≤ 2 Days)', count: statusCounts.warn, badgeBg: 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 border border-amber-200 dark:border-amber-800' },
          { key: 'ok', label: 'In-Cycle / Optimal', count: statusCounts.ok, badgeBg: 'bg-tea-100 dark:bg-tea-900/30 text-tea-700 dark:text-tea-400 border border-tea-200 dark:border-tea-800' },
        ].map(tab => (
          <button
            key={tab.key}
      ...
      {loading ? (
        <div className="flex items-center justify-center py-20 gap-2 text-slate-400">
          ...
          <span className="text-sm font-medium">Loading Plucking Intelligence...</span>
        </div>
      ) : (
        <>
          ...
          {activeField ? (
                ...
                <div>
                  <div className="flex items-center gap-3">
                    ...
                    <button
                      ...
                      className="text-xs font-semibold text-tea-600 dark:text-tea-400 hover:underline flex items-center gap-1 bg-tea-50 dark:bg-tea-900/20 px-2 py-0.5 rounded-lg border border-tea-100 dark:border-tea-800"
                    >
                      Block Details <ArrowRight size={11} />
                    </button>
                  </div>
                  <p className="text-xs text-tea-600 dark:text-tea-400 font-medium mt-0.5">Harvest Log Matrix ({formatDate(startDate)} – {formatDate(endDate)})</p>
                </div>
                    ...
                    <table className="w-full text-left border-collapse text-xs">
                      <thead>
                        <tr className="bg-slate-50 dark:bg-slate-800/60 text-slate-500 dark:text-slate-400 border-b border-slate-200 dark:border-slate-800 font-semibold">
                          ...
                          <th className="px-3 py-2 text-right w-16">Yield/Ac</th>
                        </tr>
                      </thead>
                          ...
                          return (
                            <tr
                              ...
                              className={`transition-colors ${
                                row
                                  ? 'bg-tea-50/40 dark:bg-tea-900/20 hover:bg-tea-100/60 dark:hover:bg-tea-800/40 cursor-pointer'
                                  : ''
                              } ${isNextDue ? 'ring-1 ring-amber-500/50 bg-amber-500/5' : isToday ? 'bg-slate-100 dark:bg-slate-800/50' : ''}`}
                            >
                              <td className="px-3 py-2">
                                <div className="flex flex-col">
                                  <span className={`font-semibold ${isNextDue ? 'text-amber-600' : isToday ? 'text-slate-900 dark:text-slate-100' : 'text-slate-700 dark:text-slate-300'}`}>
                                    {d.toLocaleDateString('en-US', { day: '2-digit', month: 'short' })}
                                  </span>
                                  {isToday && <span className="text-[9px] font-bold text-slate-500">Today</span>}
                                </div>
                              </td>
                              <td className="px-3 py-2">
                                {row ? (
                                  <div className="flex flex-col leading-tight">
                                    <span className="font-semibold text-tea-600 dark:text-tea-400 truncate max-w-[80px]">
                                      {row.round || "Round 1"}
                                    </span>
                                    <span className="text-[10px] text-slate-400 truncate max-w-[80px]">{row.type}</span>
                                    {row.labours > 0 && (
                                      <span className="text-[9px] text-indigo-500 font-medium">{row.labours} pax</span>
                                    )}
                                  </div>
                                ) : isNextDue ? (
                              ...
                              <td className="px-3 py-2 text-right">
                                {row ? (
                                  <div className="flex flex-col items-end leading-tight">
                                    <span className="font-bold text-tea-600 dark:text-tea-400">{row.qty}kg</span>
                                    <span className="text-[10px] text-slate-400">{row.area}ac</span>
                                    {row.labours > 0 && (
                                      <span className="text-[9px] text-tea-600/90 font-medium">{(row.qty / row.labours).toFixed(1)}k/p</span>
                                    )}
                                  </div>
                                ) : <span className="text-slate-300 dark:text-slate-700">—</span>}
                              </td>
            ...
            <div className="p-8 text-center bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl text-slate-400 text-xs font-medium">
              Please select a block to view harvest log matrix.
            </div>
          )}
        </>
      )}
          ...
          <div className="w-full max-w-md bg-white dark:bg-slate-900 h-full overflow-y-auto shadow-2xl p-6 flex flex-col justify-between border-l border-slate-200 dark:border-slate-800">
            <div>
              ...
              <div className="grid grid-cols-2 gap-3 my-6">
                <div className="p-3.5 rounded-xl bg-tea-50 dark:bg-tea-900/20 border border-tea-100 dark:border-tea-800">
                  <p className="text-xs font-medium text-tea-700 dark:text-tea-400">Total Yield</p>
                  <p className="text-xl font-bold text-slate-900 dark:text-white mt-0.5">
                    {(totals[selectedBlockForDetail.id]?.qty || 0).toFixed(1)} <span className="text-xs font-normal text-slate-400">kg</span>
                  </p>
                </div>
                <div className="p-3.5 rounded-xl bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800">
                  <p className="text-xs font-medium text-indigo-700 dark:text-indigo-400">Labour Force</p>
                  <p className="text-xl font-bold text-slate-900 dark:text-white mt-0.5">
                ...
                <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-800">
                  <p className="text-xs font-medium text-slate-500">Active Round</p>
                  <p className="text-sm font-bold text-slate-800 dark:text-slate-200 mt-0.5">
                    {totals[selectedBlockForDetail.id]?.latestRoundLabel || 'Round 1'}
                  </p>
                </div>
                <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-800">
                  ...
                  <p className="text-sm font-bold text-amber-600 dark:text-amber-400 mt-0.5">
                    {totals[selectedBlockForDetail.id]?.nextDue ? formatDate(totals[selectedBlockForDetail.id].nextDue) : 'Ready'}
                  </p>
                </div>
              </div>

              {/* Block Readiness Overview */}
              <div className="space-y-3 pt-4 border-t border-slate-200 dark:border-slate-800">
                <h4 className="text-xs font-semibold text-slate-500 flex items-center gap-2">
                  <ShieldCheck size={14} className="text-tea-600 dark:text-tea-400" /> Block Operational Readiness
                </h4>
                ...
                <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-800 space-y-2.5 text-xs">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-tea-700 dark:text-tea-400 font-bold">{intervalDays} Days Cycle</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-slate-600 dark:text-slate-400 font-medium">Total Recorded Entries</span>
                    <span className="text-slate-900 dark:text-white font-bold">{totals[selectedBlockForDetail.id]?.logCount || 0} Logs</span>
                  </div>
                  <div className="flex justify-between items-center">
                    ...
                    <span className={`uppercase text-[10px] font-bold px-2 py-0.5 rounded-full ${
                      getDueStatus(selectedBlockForDetail.id) === 'overdue'
                        ...
                        : getDueStatus(selectedBlockForDetail.id) === 'warn'
                          ...
                          : 'bg-tea-100 dark:bg-tea-900/30 text-tea-700 dark:text-tea-400 border border-tea-200 dark:border-tea-800'
                    }`}>
      ...
      {/* INTERACTIVE DAILY CROP DETAILS MODAL */}
      {selectedDayDetail && (
        <div className="fixed inset-0 z-[120] bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-2xl shadow-xl overflow-hidden border border-slate-200 dark:border-slate-800 p-6 space-y-5">
            {/* Modal Header */}
            <div className="flex justify-between items-start">
              <div>
                <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-tea-50 dark:bg-tea-900/30 text-tea-700 dark:text-tea-400 border border-tea-100 dark:border-tea-800">
                  {selectedDayDetail.row.round || 'Round 1'} • Daily Crop Log
                </span>
                ...
                <p className="text-xs text-slate-400 flex items-center gap-1.5 mt-0.5">
                  <Calendar size={13} className="text-tea-600 dark:text-tea-400" />
            ...
            {/* Primary Crop Metrics */}
            <div className="grid grid-cols-2 gap-3">
              <div className="p-3.5 rounded-xl bg-tea-50 dark:bg-tea-900/20 border border-tea-100 dark:border-tea-800">
                <div className="flex items-center gap-1.5 text-tea-700 dark:text-tea-400 mb-0.5">
                  <Sprout size={14} />
                  <span className="text-xs font-medium">Harvest Yield</span>
                </div>
                <p className="text-xl font-bold text-slate-900 dark:text-white">
                  {selectedDayDetail.row.qty} <span className="text-xs font-normal text-slate-400">kg</span>
                </p>
              </div>
            ...
            {/* Secondary Crop Metrics */}
            <div className="space-y-2 p-3.5 rounded-xl bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-800 text-xs font-semibold">
              <div className="flex justify-between items-center">
                <span className="text-slate-500 font-normal">Plucked Area</span>
                <span className="text-slate-900 dark:text-white">
                  {selectedDayDetail.row.area} Ac <span className="text-[10px] text-slate-400 font-normal">({selectedDayDetail.block?.acres ? ((selectedDayDetail.row.area / parseFloat(selectedDayDetail.block.acres)) * 100).toFixed(0) : 0}% of block)</span>
                </span>
              </div>

              <div className="flex justify-between items-center">
                <span className="text-slate-500 font-normal">Worker Plucking Efficiency</span>
                <span className="text-tea-700 dark:text-tea-400">
                  {selectedDayDetail.row.labours > 0 ? (selectedDayDetail.row.qty / selectedDayDetail.row.labours).toFixed(1) : '0'} kg / PAX
                </span>
              </div>

              <div className="flex justify-between items-center">
                <span className="text-slate-500 font-normal">Harvest Yield Density</span>
                <span className="text-slate-900 dark:text-white">
                  {selectedDayDetail.row.area > 0 ? (selectedDayDetail.row.qty / selectedDayDetail.row.area).toFixed(1) : '0'} kg / Ac
                </span>
              </div>

              <div className="flex justify-between items-center">
                <span className="text-slate-500 font-normal">Area Coverage Rate</span>
                <span className="text-slate-900 dark:text-white">
          ...
          <div className="w-full max-w-md max-h-[90vh] overflow-y-auto bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 p-6 space-y-5">
            ...
            <div className="flex justify-between items-center border-b border-slate-100 dark:border-slate-800 pb-3">
              <div className="flex items-center gap-2">
                <div className="p-2 rounded-xl bg-tea-50 dark:bg-tea-900/20 text-tea-600 dark:text-tea-400">
                  <Settings size={18} />
                </div>
                <div>
                  <h3 className="text-base font-bold text-slate-900 dark:text-white">
                    Round Monitor Settings
                  </h3>
                  <p className="text-xs text-slate-400">Configure harvest cycle & display options</p>
                </div>
              </div>
              <Button
                variant="ghost"
                  ...
                  <button
                    ...
                    className={`h-9 rounded-xl text-xs font-semibold border transition-all ${
                      intervalDays === item.val
                        ? 'bg-tea-50 dark:bg-tea-900/30 text-tea-700 dark:text-tea-400 border-tea-500 font-bold shadow-sm'
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-3 gap-2">
                {[
                  { mode: 'combined', label: 'Combined', icon: LayoutGrid },
                  { mode: 'grid', label: 'Grid View', icon: LayoutGrid },
                  { mode: 'matrix', label: 'Matrix View', icon: Table },
                ].map(v => {
                  const Icon = v.icon;
                  return (
                    <button
                      ...
                      className={`h-9 rounded-xl text-xs font-semibold border flex items-center justify-center gap-1.5 transition-all ${
                        viewMode === v.mode
                          ? 'bg-tea-50 dark:bg-tea-900/30 text-tea-700 dark:text-tea-400 border-tea-500 font-bold shadow-sm'
                          : 'bg-slate-50 dark:bg-slate-950 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-800 hover:bg-slate-100 dark:hover:bg-slate-800'
                      }`}
            ...
            <div className="space-y-2 pt-2 border-t border-slate-100 dark:border-slate-800">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                >
                  <FileSpreadsheet size={15} className="text-tea-600 dark:text-tea-400" />
            ...
            {/* Monthly Round Calculation Guidelines */}
            <div className="space-y-2 pt-2 border-t border-slate-100 dark:border-slate-800">
              <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide flex items-center gap-1.5">
                <ShieldCheck size={14} className="text-tea-600 dark:text-tea-400" />
                Monthly Round Standard Guidelines
              </label>
              <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 space-y-2.5 text-xs text-slate-600 dark:text-slate-300">
                <div className="flex items-start gap-2">
                  <div className="w-1.5 h-1.5 rounded-full bg-tea-500 mt-1.5 shrink-0" />
                  <p>
                    <strong className="text-slate-900 dark:text-white">Monthly Reset:</strong> The first plucking date in any given calendar month resets to <span className="font-semibold text-tea-600 dark:text-tea-400">Round 01</span>.
                  </p>
                </div>
                <div className="flex items-start gap-2">
                  <div className="w-1.5 h-1.5 rounded-full bg-tea-500 mt-1.5 shrink-0" />
                  <p>
                    <strong className="text-slate-900 dark:text-white">Round Progress:</strong> Subsequent plucking dates in that month increment (<span className="font-semibold text-slate-700 dark:text-slate-200">Round 02, Round 03</span>, etc.) based on your chosen cycle interval.
                  </p>
                </div>
                <div className="flex items-start gap-2">
                  <div className="w-1.5 h-1.5 rounded-full bg-tea-500 mt-1.5 shrink-0" />
                  <p>
                    <strong className="text-slate-900 dark:text-white">Month-End Round:</strong> The final plucking date of the month represents the relevant last round of that month.
                  </p>
                </div>
                <div className="flex items-start gap-2">
                  <div className="w-1.5 h-1.5 rounded-full bg-tea-500 mt-1.5 shrink-0" />
                  <p>
                    <strong className="text-slate-900 dark:text-white">Next Due Prediction:</strong> If the next projected date falls in the same month, Next Round shows <span className="font-semibold text-amber-600 dark:text-amber-400">Round N+1</span>. If it falls into the next month, Next Round automatically predicts <span className="font-semibold text-tea-600 dark:text-tea-400">Round 01</span>.
                  </p>
                </div>
              </div>
            </div>
