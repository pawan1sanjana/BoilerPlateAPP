...
const MANURE_TYPES = [
  { id: 't65', label: 'T65 (NPK 12-6-20)', color: 'bg-sky-500' },
  { id: 't200', label: 'T200 (NPK 15-5-26)', color: 'bg-amber-500' },
  { id: 't750', label: 'T750 (Organic Blend)', color: 'bg-emerald-500' },
  { id: 'u709', label: 'U709 (High Nitrogen)', color: 'bg-violet-500' },
  { id: 'u834', label: 'U834 (Magnesium Fortified)', color: 'bg-rose-500' },
];
...
export default function ManureRound() {
  ...
  const [statusTab, setStatusTab] = useState<'all' | 'weeding_required' | 'overdue' | 'warn' | 'ok'>('all');
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [logsRes, musterRes, weedRes] = await Promise.all([
      ...
      if (logsRes.data && logsRes.data.length > 0) {
        ...
        const resYearLogs = await apiClient.get(`/crop/manure-logs/year?year=${reqYear}`);
        if (resYearLogs.success && resYearLogs.data) {
          logsData = resYearLogs.data;
        }
      }

      const weedLogsByBlock: Record<string, any[]> = {};
      (weedRes.data || []).forEach((w: any) => {
        const bId = String(w.block_id);
        if (!weedLogsByBlock[bId]) weedLogsByBlock[bId] = [];
        weedLogsByBlock[bId].push(w);
      });

      (musterRes.data || []).forEach((m: any) => {
        if (m.task && String(m.task).toLowerCase().includes('weed')) {
          const bId = String(m.block_id);
          if (!weedLogsByBlock[bId]) weedLogsByBlock[bId] = [];
          weedLogsByBlock[bId].push({
      ...
      const dbMusterManure = (musterRes.data || []).filter((m: any) =>
        m.task && (String(m.task).toLowerCase().includes('manure') || String(m.task).toLowerCase().includes('fertilizer'))
      );
      ...
      const manureLogDatesByBlock = new Set<string>();
      ...
      (logsData || []).forEach((log: any) => {
        ...
        manureLogDatesByBlock.add(`${bId}_${dateKey}`);

        logsByBlock[bId].push({
          ...log,
          date: dateKey,
          area: Number(log.acres_covered || log.area || 0),
          qty: Number(log.qty || log.total_kg || 0),
          labours: Number(log.workers || log.labours || (log.worker_id ? 1 : 0)) || 1,
          type: MANURE_TYPES.find(t => t.id === log.type)?.label || log.type || 'T65 Fertilizer'
        });
      });

      (dbMusterManure || []).forEach((m: any) => {
        const bId = String(m.block_id);
        ...
        if (manureLogDatesByBlock.has(`${bId}_${dateKey}`)) return;
      ...
      mappedFields.forEach((f: any) => {
        ...
        wLogs.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
        ...
        if (logs.length > 0) {
          ...
          if (!tot.lastWeedDate) {
            tot.postWeedStatus = 'weeding_required';
          } else if (new Date(tot.lastAppDate).getTime() >= new Date(tot.lastWeedDate).getTime()) {
            tot.postWeedStatus = 'applied';
          } else {
            tot.postWeedStatus = 'pending';
          }
          ...
          tot.postWeedStatus = tot.lastWeedDate ? 'pending' : 'weeding_required';
        }
        totalsMap[f.id] = tot;
      });
      ...
      console.error("Error loading manure round data:", e);
    } finally {
      setLoading(false);
    }
  };
  ...
  const statusCounts = useMemo(() => {
    const counts = { all: fields.length, weeding_required: 0, overdue: 0, warn: 0, ok: 0 };
    fields.forEach(f => {
      const tot = totals[f.id];
      if (tot?.postWeedStatus === 'weeding_required') counts.weeding_required++;
  ...
  const filteredFields = useMemo(() => {
    return fields.filter(f => {
      ...
      const st = getDueStatus(f.id);
      const tot = totals[f.id];
      let matchesTab = statusTab === 'all';
      if (statusTab === 'weeding_required') matchesTab = tot?.postWeedStatus === 'weeding_required';
      else if (statusTab === 'overdue') matchesTab = st === 'overdue';
      else if (statusTab === 'warn') matchesTab = st === 'warn';
      else if (statusTab === 'ok') matchesTab = st === 'ok';
  ...
  const exportToExcel = () => {
    const data = filteredFields.map(f => {
      ...
      return {
        'Block': f.label,
        'GIS Area (Ac)': f.acres,
        'Applied Area (Ac)': (tot.area || 0).toFixed(2),
        'Total Manure Qty (Kg)': (tot.qty || 0).toLocaleString(),
        'Labour Deployment': tot.labours || 0,
        'Weeding Status': tot.postWeedStatus === 'weeding_required' ? 'Weeding Incomplete' : tot.lastWeedDate ? `Completed (${tot.lastWeedDate})` : 'Completed',
        'Last Application': tot.lastAppDate || '-',
        'Next Due Date': tot.nextDue || 'Ready',
        'Next Round': tot.nextRoundLabel || 'Round 01',
        'Status': getDueStatus(f.id).toUpperCase()
      };
    });
    const ws = XLSX.utils.json_to_sheet(data);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Manure Rounds");
    XLSX.writeFile(wb, `manure_round_report_${startDate}_to_${endDate}.xlsx`);
  ...
  const exportToPDF = () => {
    ...
    doc.text("Manure Application Round Monitor Report", 14, 20);
    ...
    const tableData = filteredFields.map(f => {
      ...
      return [
        ...
        `${(tot.qty || 0).toLocaleString()} Kg`,
        tot.postWeedStatus === 'weeding_required' ? 'Weeding Needed' : 'Weeding Done',
    ...
    autoTable(doc, {
      startY: 40,
      head: [['Block', 'Total Area', 'Applied Area', 'Manure Qty', 'Weeding Status', 'Next Due', 'Status']],
      body: tableData,
      theme: 'grid',
    ...
    doc.save(`manure_round_report_${startDate}_to_${endDate}.pdf`);
    setShowSettings(false);
  };
  ...
  return (
    <div className="pb-16 space-y-4">
      ...
      <div className="flex flex-col gap-1">
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">Manure Round Monitor</h1>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          },
          {
            ...
            val: filteredTotals.qty.toLocaleString(),
            unit: "Kg",
            icon: Activity,
      ...
      <div className="flex items-center gap-1.5 overflow-x-auto pb-1 bg-slate-50 dark:bg-slate-950 p-1.5 rounded-xl border border-slate-200 dark:border-slate-800">
        {[
          ...
          { key: 'weeding_required', label: 'Weeding Incomplete', count: statusCounts.weeding_required, badgeBg: 'bg-rose-100 dark:bg-rose-900/30 text-rose-700 dark:text-rose-400 border border-rose-200 dark:border-rose-800' },
      ...
      {loading ? (
        <div className="flex items-center justify-center py-20 gap-2 text-slate-400">
          ...
          <span className="text-sm font-medium">Loading Manure Intelligence...</span>
        </div>
      ) : (
        <>
          ...
          {activeField ? (
            <div className="bg-white dark:bg-slate-900 rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-800 shadow-sm">
              <div className="px-5 py-4 bg-slate-50 dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center flex-wrap gap-2">
                <div>
                  ...
                  <p className="text-xs text-emerald-600 dark:text-emerald-400 font-medium mt-0.5">Manure Log Matrix ({formatDate(startDate)} – {formatDate(endDate)})</p>
                </div>
              ...
              {/* Post-Weeding Manure Schedule Banner */}
              {totals[activeField!]?.postWeedStatus === 'weeding_required' && (
                <div className="px-5 py-2.5 bg-red-50 dark:bg-red-950/40 border-b border-red-200 dark:border-red-800/80 flex justify-between items-center flex-wrap gap-2 text-xs">
                  <div className="flex items-center gap-2 text-red-800 dark:text-red-300 font-medium">
                    <AlertCircle size={15} className="text-red-500 shrink-0" />
                    <span>Prerequisite Warning: <strong>Weeding is NOT completed for this block.</strong> Weeding must be completed before applying manure.</span>
                  </div>
                  <button
                    onClick={() => navigate('/rounds/weeding')}
                    className="px-3 py-1 bg-red-600 hover:bg-red-700 text-white rounded-lg font-bold text-[11px] transition-colors flex items-center gap-1 shrink-0 shadow-sm"
                  >
                    Go to Weeding Round <ArrowUpRight size={12} />
                  </button>
                </div>
              )}

              {totals[activeField!]?.postWeedStatus === 'applied' && (
                <div className="px-5 py-2 bg-emerald-50 dark:bg-emerald-950/40 border-b border-emerald-200 dark:border-emerald-800/80 flex justify-between items-center flex-wrap gap-2 text-xs">
                  <div className="flex items-center gap-2 text-emerald-800 dark:text-emerald-300 font-medium">
                    ...
                    <span>Post-Weeding Fertilizer: <strong>Weeding Completed & Manure Applied on {formatDate(totals[activeField!].lastAppDate)}</strong>.</span>
                  </div>
                  <button
                    onClick={() => navigate('/rounds/weeding')}
                  ...
                  >
                    View Weeding Monitor <ArrowUpRight size={11} />
                  </button>
                </div>
              )}

              {totals[activeField!]?.postWeedStatus === 'pending' && (
                <div className="px-5 py-2.5 bg-amber-50 dark:bg-amber-950/40 border-b border-amber-200 dark:border-amber-800/80 flex justify-between items-center flex-wrap gap-2 text-xs">
                  <div className="flex items-center gap-2 text-amber-800 dark:text-amber-300 font-medium">
                    ...
                    <span>Post-Weeding Requirement: <strong>Weeding completed on {formatDate(totals[activeField!].lastWeedDate)}</strong>. Manure application is scheduled post-weeding.</span>
                  </div>
                  <button
                  ...
                  >
                    Log Manure Application <ArrowUpRight size={12} />
                  </button>
                </div>
              )}
            ...
            <div className="p-8 text-center bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl text-slate-400 text-xs font-medium">
              Please select a block to view manure log matrix.
            </div>
          )}
        </>
      )}
              ...
              <div className="py-6 space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div className="p-3.5 rounded-xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800">
                    <p className="text-xs text-emerald-600 dark:text-emerald-400 font-medium">Total Applied Area</p>
                  ...
                  <div className="p-3.5 rounded-xl bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-800 col-span-2">
                    <p className="text-xs text-amber-600 dark:text-amber-400 font-medium">Total Quantity Applied</p>
                    <p className="text-xl font-bold text-slate-900 dark:text-white mt-0.5">
                      {(totals[selectedBlockForDetail.id]?.qty || 0).toLocaleString()} <span className="text-xs font-normal text-slate-400">Kg</span>
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-3 pt-4 border-t border-slate-200 dark:border-slate-800">
                  ...
                  <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-800 space-y-2.5 text-xs">
                    ...
                    <div className="flex justify-between items-center">
                      <span className="text-slate-600 dark:text-slate-400 font-medium">Post-Weeding Schedule</span>
                      <span className={`font-bold flex items-center gap-1 ${
                        totals[selectedBlockForDetail.id]?.postWeedStatus === 'applied'
                          ? 'text-emerald-600 dark:text-emerald-400'
                          : totals[selectedBlockForDetail.id]?.postWeedStatus === 'pending'
                        ...
                        {totals[selectedBlockForDetail.id]?.postWeedStatus === 'applied'
                          ? 'Completed & Applied'
                          : totals[selectedBlockForDetail.id]?.postWeedStatus === 'pending'
                            ? 'Scheduled Following Weeding'
                            : 'Weeding Incomplete (Required)'}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
          ...
          <div className="w-full max-w-md bg-white dark:bg-slate-900 h-full overflow-y-auto shadow-2xl p-6 flex flex-col justify-between border-l border-slate-200 dark:border-slate-800 space-y-6">
            <div>
              ...
              <div className="flex justify-between items-center pb-4 border-b border-slate-200 dark:border-slate-800">
                <div>
                  <span className="text-[10px] font-bold uppercase tracking-wider text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/30 px-2 py-0.5 rounded-full border border-emerald-200 dark:border-emerald-800">
                    {selectedDayDetail.row.round || 'Manure Log'}
                  </span>
              ...
              <div className="grid grid-cols-2 gap-3 mt-6">
                <div className="p-3.5 rounded-xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800">
                  <div className="flex items-center gap-1.5 text-emerald-700 dark:text-emerald-400 mb-0.5">
                    ...
                    <span className="text-xs font-medium">Applied Area</span>
                  </div>
              ...
              <div className="space-y-2.5 p-3.5 rounded-xl bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-800 text-xs font-semibold mt-4">
                <div className="flex justify-between items-center">
                  <span className="text-slate-500 font-normal">Manure Type</span>
                  <span className="text-slate-900 dark:text-white font-bold">{selectedDayDetail.row.type}</span>
                </div>

                <div className="flex justify-between items-center">
                  <span className="text-slate-500 font-normal">Manure Quantity</span>
                  <span className="text-indigo-600 dark:text-indigo-400 font-bold">{selectedDayDetail.row.qty} Kg</span>
                </div>
          ...
          <div className="w-full max-w-md max-h-[90vh] overflow-y-auto bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 p-6 space-y-5">
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="text-base font-bold text-slate-900 dark:text-white">
                    Manure Round Settings
                  </h3>
                  <p className="text-xs text-slate-400">Configure manure cycle & display options</p>
                </div>
              </div>
              <Button
                variant="ghost"
            ...
            <div className="space-y-2 pt-2 border-t border-slate-100 dark:border-slate-800">
              <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide flex items-center gap-1.5">
                ...
                Annual Manure Round Guidelines
              </label>
              <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 space-y-2.5 text-xs text-slate-600 dark:text-slate-300">
                <div className="flex items-start gap-2">
                  ...
                  <p>
                    <strong className="text-slate-900 dark:text-white">Annual Reset:</strong> The first manure application date in any given calendar year resets to <span className="font-semibold text-emerald-600 dark:text-emerald-400">Round 01</span>.
                  </p>
                </div>
                <div className="flex items-start gap-2">
                  <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 mt-1.5 shrink-0" />
                  <p>
                    <strong className="text-slate-900 dark:text-white">Round Progress:</strong> Subsequent manure dates in that year increment (<span className="font-semibold text-slate-700 dark:text-slate-200">Round 02, Round 03</span>, etc.) after every 90 days.
                  </p>
                </div>
                <div className="flex items-start gap-2">
                  <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 mt-1.5 shrink-0" />
                  <p>
                    <strong className="text-slate-900 dark:text-white">Year-End Round:</strong> The final manure date of the year represents the relevant last round of that year.
                  </p>
                </div>
                <div className="flex items-start gap-2">
                  <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 mt-1.5 shrink-0" />
                ...
                <div className="flex items-start gap-2">
                  <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 mt-1.5 shrink-0" />
                  <p>
                    <strong className="text-slate-900 dark:text-white">Post-Weeding Prerequisite:</strong> Weeding <span className="font-semibold text-emerald-600 dark:text-emerald-400">must be completed prior to manure application</span> in each round for effective nutrient uptake.
                  </p>
                </div>
              </div>
            </div>
