  ...
  const load = async () => {
    ...
    try {
      ...
      if (aRes.success && Array.isArray(aRes.data)) {
        setAccounts(aRes.data.filter((a: any) => a.isActive && a.type === 'income'));
        // Preselect first income account if exists
        const firstIncome = aRes.data.find((a: any) => a.type === 'income');
  ...
  const filtered = useMemo(() => {
    ...
    if (q) {
      result = result.filter(i =>
        ...
        String(i.incomeAccountCode || '').toLowerCase().includes(q) ||
        String(i.incomeAccountName || '').toLowerCase().includes(q)
      );
    }
    return result;
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-900 dark:text-white">Income Analysis</h1>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Track revenue sources, auction proceeds, green leaf sales, and general income postings</p>
        </div>
        ...
        <div className="premium-card p-5">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            ...
            <div className="text-right w-full md:w-auto">
              ...
              <p className="text-sm font-black text-emerald-600 dark:text-emerald-400">+{totalShown.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
            </div>
          </div>
        </div>
                    ...
                    <tr key={i.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                      ...
                      <td className="py-3 px-6 text-right">
                        <span className="text-[11px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 group-hover:bg-emerald-100 dark:group-hover:bg-emerald-900/40 transition-colors inline-block">
                          {Number(i.amount).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                        </span>
                      </td>
                    </tr>
                  ))}
