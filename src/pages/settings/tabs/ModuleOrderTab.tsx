...
import { 
  ...
  Calculator, Droplets, Ruler, Truck, Building2, Bot
} from 'lucide-react'
...
const MODULE_DISPLAY_INFO: Record<string, { name: string; icon: React.ReactNode }> = {
  ...
  'chatbot': { name: 'AI Assistant', icon: <Bot size={16} /> },
  ...
  'calculators': { name: 'Calculators', icon: <Calculator size={16} /> },
  'calculators/ph': { name: 'PH Dolomite', icon: <Calculator size={16} /> },
  'calculators/foliar': { name: 'Foliar Spray', icon: <Droplets size={16} /> },
  'calculators/units': { name: 'Units Converter', icon: <Ruler size={16} /> },
  'estates_management': { name: 'Estates & Factories', icon: <Truck size={16} /> },
  'estates_management/estates': { name: 'Estates List', icon: <Truck size={16} /> },
  'estates_management/factories': { name: 'Factories List', icon: <Building2 size={16} /> }
}
  ...
  return (
    <div className="space-y-6 animate-in fade-in zoom-in-95 duration-300">
      ...
      {draftMainOrder.map((mainKey) => {
        ...
        const mainInfo = MODULE_DISPLAY_INFO[mainKey] || { name: mainKey, icon: null }
        ...
        return (
          <div key={`sub-${mainKey}`} className="bg-white dark:bg-slate-900/50 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 pl-10 border-l-4 border-l-blue-500 shadow-sm relative overflow-hidden">
            ...
            <h4 className="text-xs font-bold text-slate-400 dark:text-slate-500 mb-4 uppercase tracking-widest flex items-center gap-2 relative z-10">
              {mainInfo.icon && React.isValidElement(mainInfo.icon) ? React.cloneElement(mainInfo.icon as React.ReactElement<any>, { size: 14 }) : <SettingsIcon size={14} />} Nested Items: {mainInfo.name}
            </h4>
